...
def _contents(chapters: list[str], slide_no: int, total: int) -> str:
    ...
    for i, t in enumerate(chapters[:6]):
      ...
      <div class="concept-box">
        ...
        <h4>{_rich(t)}</h4>
        <p style="font-size:14px;color:var(--text-2);line-height:1.55;margin-top:6px">本部分提炼原文的关键观点。</p>
      </div>
            """
        )
...
def _cards(slide: dict[str, Any], chapters: list[str], current_idx: int, slide_no: int, total: int) -> str:
    ...
    for i, bullet in enumerate(bullets[:6]):
      ...
      <div class="concept-box">
        ...
        <p style="font-size:14px;color:var(--text-2);line-height:1.55;margin-top:6px">{_rich(body if head else "来自原文的核心要点提炼。")}</p>
      </div>
            """
        )
    if not cells:
