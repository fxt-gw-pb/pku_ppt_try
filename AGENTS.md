...
MVP web service that turns a Chinese academic manuscript into a downloadable HTML deck. PKU red is the default theme but the pipeline is multi-template — 16 templates total (PKU + 15 imported html-ppt themes).
...
manuscript + template_id
  ...
  -> src/schema/      validate generic slide_json
  -> src/renderer/    dispatch.py picks the dedicated renderer; PKU uses compile_to_pku() instead
  -> src/exporter/    copy template bundle from templates/html-ppt/<id>/ or pku-red-defense-ppt/assets/template/
  -> zip + static preview served by server/app.py at /decks/<id>/{index.html,zip}
```

The repository intentionally contains these related surfaces:

- Backend: `server/app.py`, FastAPI, job state in `data/jobs/`, generated decks in `outputs/`. Static mounts: `/decks`, `/previews`, `/收款码`, `/`.
- Frontend: root `index.html` for GitHub Pages plus `web/` for local FastAPI static serving. Two-view static app; per-card `预览` button links to committed sample decks under `previews/<id>/`.
...
- 15 html-ppt full-deck templates under `templates/html-ppt/<id>/` with shared assets in `templates/html-ppt/shared/assets/`. Each has a dedicated renderer in `src/renderer/<id_slug>.py`.
- Committed per-template sample decks under `previews/<id>/`, regenerated by `python scripts/build_previews.py`.
...
These apply **only to the `pku-red` template**. The other 15 html-ppt templates each carry their own theme and may use dark backgrounds, gradients, neon accents, sticker UI, etc.

- Canvas is 16:9, 1280×720 across all templates.
- PKU template: white content pages; PKU red `#9A0000` is the only strong accent. Red is reserved for nav highlights, ordinal numbers, key phrases, section-divider backgrounds, and cover/closing backgrounds. Cards use light borders and white fills; avoid heavy shadows. No glassmorphism, purple/blue tech gradients, dark hero pages, or landing-page card stacks. Charts, screenshots, and diagrams must use `contain` or `diagram`, not cropped `cover`.

Reference files (PKU only):
...
## Typography rule (all html-ppt templates)

Headlines across the 15 html-ppt templates follow a unified CJK-first scale on the 1280×720 canvas:

- Cover headline (`.h1` / template-prefixed equivalents): **56–74px** CJK default, line-height ~1.16, near-zero `letter-spacing`, `max-width:1040–1080px`, `word-break:break-word`.
- Slide title (`.h2`): 40–48px, line-height ~1.2.
- Watermark numerals (`.section-num`, `.dk-big`, `.kb-big-num`, `.hc-big`, `.gd-big`, `.oc-big`, `.mega`): capped at 144–180px.
- Each headline carries a `:lang(en)` override restoring 74–96px Latin-keynote sizes.

When editing or adding a template's CSS:

- Don't use `letter-spacing` more aggressive than `-0.5px` on the CJK default rule.
- Always set `max-width` on hero headlines that can wrap.
- If you change a `.slide > *` style, exclude decorative absolute-positioned children (e.g. `:not(.section-num):not(.cover-bg):not(.cover-blob)`) so the watermark stays positioned where it should.

## PDF / Print

Every generated deck and preview carries a floating **导出 PDF** pill (bottom-right) that calls `window.print()`. Save-as-PDF in the print dialog produces a one-page-per-slide PDF at native 1280×720.

- html-ppt decks: button injected by `templates/html-ppt/shared/assets/runtime.js`; print rules in `templates/html-ppt/shared/assets/base.css`. Keyboard shortcut **P**.
- PKU decks: button injected by `pku-red-defense-ppt/assets/template/assets/runtime.js` (mirrored to root `assets/runtime.js`); print rules already live in `deck-stage.js`'s shadow-DOM `@media print`; the `@page` size is injected via `<style id="deck-stage-print-page">` because `@page` has no effect inside shadow DOM.

Shared print stack: `@page { size:1280px 720px; margin:0 }` + `-webkit-print-color-adjust:exact` on every element + `.slide { width:1280px; height:720px; page-break-after:always }` + `.deck { overflow:visible }`. Verified with headless Chrome (`--print-to-pdf`): both pitch-deck and pku-red produce 14-page PDFs at 960×540 PDF points (= 1280×720 CSS px).
...
- `src/renderer/html_ppt_generic.py`: generic fallback (kept for safety; currently unused).
...
- `scripts/build_previews.py`: rebuild all per-template sample decks under `previews/<id>/`.
- `templates/html-ppt/shared/assets/base.css`: shared deck CSS, including `@media print` for PDF export.
- `templates/html-ppt/shared/assets/runtime.js`: keyboard runtime, presenter mode, 导出 PDF button injection.
...
- `pku-red`: default, PKU JSON runtime via `compile_to_pku()`.
- 15 imported full-deck templates: `xhs-white-editorial`, `graphify-dark-graph`, `knowledge-arch-blueprint`, `hermes-cyber-terminal`, `obsidian-claude-gradient`, `testing-safety-alert`, `xhs-pastel-card`, `dir-key-nav-minimal`, `pitch-deck`, `product-launch`, `tech-sharing`, `weekly-report`, `xhs-post`, `course-module`, `presenter-mode-reveal`.

All 15 html-ppt templates have dedicated renderers under `src/renderer/<slug>.py`, dispatched by `src/renderer/dispatch.py`. The generic fallback `src/renderer/html_ppt_generic.py` is kept for safety but currently unused.
...
After renderer or template-CSS changes, also rebuild the committed sample decks:
...
For PDF/print regressions, render one deck with headless Chrome:
...
# expect 14 pages at 960×540 PDF points
```

For template changes, also preview the deck over HTTP, not `file://`, because `runtime.js` fetches `data/slide.json`.
