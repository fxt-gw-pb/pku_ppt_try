...
│
├── index.html                  # GitHub Pages 站点首页（粘贴文稿 → 生成 PPT 的前端）
├── demo.html                   # 原始 PKU 模板 deck（独立可运行的示例）
...
- 一个文本框，粘贴文稿。
- 一个模板选择区，当前支持 **北大红答辩模板** 与 **小红书白底杂志风**。
- 一个 **生成 PPT** 按钮。
- 实时显示任务状态。
...
当前后端通过 `template_id` 并行管理不同模板：

| template_id | 模板 | 引擎 | 模板源 |
|-------------|------|------|--------|
...
**CORS**：当前 `server/app.py` 只允许 GitHub Pages 域名和本地开发端口。新增前端域名时用 `CORS_ORIGINS="a,b,c"` 配置，不要直接放宽为 `["*"]`。
...
仓库根目录的 `demo.html` + `deck-stage.js` + `data/slides.json` 是一套独立可运行的示例 deck，不需要后端：
...
> ⚠️ `index.html` 现在是前端站点（GitHub Pages 首页）。示例 deck 改在 `demo.html`。

可以直接修改 `data/slides.json` 替换为你的内容，不走 LLM 流水线。详见 `pku-red-defense-ppt/SKILL.md`。

GitHub Pages 部署后访问：
- `https://fxt-gw-pb.github.io/pku_ppt_try/` → 前端站点
- `https://fxt-gw-pb.github.io/pku_ppt_try/demo.html` → 示例 deck
